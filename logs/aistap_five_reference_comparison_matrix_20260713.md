...
|---|---|---|---|
| 绗簩鎵? | Submission closure | AISTAP is not yet at final submission closure | Keep the package authoritative and prevent draft drift |
| 绗簩鎵? | Claim hygiene and validation layering | AISTAP boundary is good, but the evidence is still sparse | Add dense CFAR, target-preservation, and stress evidence |
| 绗笁鎵? | Strong-baseline acquisition ledger | AISTAP has scaffold evidence, but not a trainability check | Add a minimal trainability check and keep the acquisition matrix explicit |
