...
% Bibliographic keys are placeholders and should be replaced by verified IEEE-style references.
...
Modern remote-sensing detectors also inherit architectures and evaluation habits from generic object detection and segmentation. Two-stage detectors, one-stage detectors, fully convolutional segmentation networks, and encoder-decoder designs have all influenced remote-sensing detection pipelines \cite{ren2017fasterrcnn,redmon2016yolo,shelhamer2017fcn,ronneberger2015unet,liu2020objectsurvey}. These architectures are useful, but their standard metrics do not automatically translate into radar low-false-alarm operation. TP-SSCS therefore uses learning only as a compact target-preserving score component and retains detector operating curves as the main evaluation language.
...
The rank dependence has a practical implication for method design. A single residual map is not a universal replacement for the raw observation. The detector may need a conservative residual at one operating point and a more aggressive residual at another. Instead of selecting rank only by reconstruction error or visual residual quality, the policy should be selected by the target-preserving operating curve. TP-SSCS uses this insight by keeping rank 30 for the manuscript-facing low-false-alarm branch while adding a gate that reduces the target-loss penalty. Fig.~\ref{fig:target-preserving-principle} illustrates this suppression-preservation conflict and the intended TP-SSCS operating compromise.

\begin{figure*}[!t]
\centering
\FigureOrPlaceholder{../figures/submitted/figure2_target_preserving_principle.png}{0.92\textwidth}
\caption{Target-preserving principle for structured-clutter suppression. Aggressive residual suppression can reduce clutter while attenuating weak targets; TP-SSCS seeks an operating compromise that preserves target evidence and improves low-false-alarm detection.}
\label{fig:target-preserving-principle}
\end{figure*}
...
This boundary also defines what would falsify or weaken the method. If TP-SSCS improved residual appearance but failed to improve calibrated $P_{\mathrm{D}}$, the detector claim would fail. If it improved only on the development sample but not on official full-test assets, the result would remain a diagnostic rather than a full detector policy. If it transferred directly to IPIX poorly, that result must be reported rather than hidden; in this study, direct zero-shot IPIX transfer is indeed negative. The positive claims are therefore restricted to the protocols that actually pass. Fig.~\ref{fig:target-preserving-principle} is used here as a boundary figure rather than as a proof figure: it states the preservation-suppression compromise that the subsequent experiments must verify numerically.
...
The training objective is interpreted operationally rather than as pure reconstruction. The gate should reduce target loss, preserve useful residual contrast, and avoid score behavior that collapses under perturbation. This is why the selected branch is not chosen solely by the highest public-sample $P_{\mathrm{D}}$ at one operating point. It is chosen because it provides a favorable target-loss and detection trade-off, remains finite across seeds, and can be carried into a fixed-policy validation protocol. The system-level flow in Fig.~\ref{fig:tpsscs-architecture} should therefore be read as a detector-policy architecture: the raw and residual branches remain visible until the final false-alarm-calibrated decision.

The small hidden width is also a design decision. A larger model could fit the public-sample diagnostics more aggressively, but it would increase the risk of learning sample-specific artifacts. The purpose of the branch is to implement a target-preservation correction to a radar residual score, not to replace the detector with a high-capacity black box. This makes the branch easier to audit and makes negative external results more interpretable. Fig.~\ref{fig:local-gating} details the local gating mechanism that chooses between residual emphasis and raw-evidence preservation.

\begin{figure*}[!t]
\centering
\FigureOrPlaceholder{../figures/submitted/figure4_local_gating_mechanism.png}{0.92\textwidth}
\caption{Local adaptive target-preservation gating mechanism. The gate uses local raw intensity, residual magnitude, and local contrast to decide whether to emphasize residual evidence or preserve raw evidence before target-preserving fusion.}
\label{fig:local-gating}
\end{figure*}
...
Fig.~\ref{fig:experimental-protocol} summarizes the complete protocol and evidence mapping across the three radar-domain datasets. Table~\ref{tab:dataset-protocol} gives the corresponding unit definitions and claim boundaries used for the quantitative evaluation.

\begin{figure*}[!t]
\centering
\FigureOrPlaceholder{../figures/submitted/figure5_experimental_protocol.png}{0.92\textwidth}
\caption{Experimental protocol and evidence mapping. Public AISTAP-SIM samples are used for development and target-loss audits, official AISTAP-SIM full-test assets provide fixed-policy in-domain validation, IPIX supports validation-selected external fusion, and SSDD supports supervised external adaptation.}
...
The public AISTAP-SIM sample and the official full-test assets serve different roles. The public sample is appropriate for understanding mechanism because it contains target-only information that supports target-loss diagnostics. It is not sufficient by itself to establish a full detector claim because the evaluated target-bearing sample is small. The official full-test assets provide the scale needed for the main in-domain validation. The same saved detector policy is applied to both official assets so that the result tests cross-condition behavior within AISTAP-SIM rather than retuned per-asset performance. Fig.~\ref{fig:experimental-protocol} should be read together with this split: it separates mechanism development, validation-selected adaptation, and final held-out testing before any operating-curve claims are made.
...
The current draft intentionally keeps references as placeholders until they are verified. This avoids a common manuscript-preparation error: inserting plausible but unverified citations for STAP, CFAR, AISTAP-SIM, IPIX, and SSDD. Before submission, the placeholder keys must be replaced with DOI-verified IEEE-style references. This does not affect the experimental claims, but it affects manuscript readiness.
...
The branch does not claim to dominate every possible residual setting. Its role is more specific: it reduces the target-loss penalty of the low-rank residual comparator while maintaining similar public-sample detection behavior. This is the trade-off required for low-false-alarm detection. If the branch had improved target loss but collapsed detection probability, it would not support the detector claim. If it had improved detection probability only by increasing target loss, it would not solve the preservation problem. The selected branch avoids both failure modes within the public-sample diagnostic setting. Table~\ref{tab:gate-preservation-summary} condenses the branch-level evidence and separates deployable trainable behavior from oracle diagnostics.

\begin{table*}[!t]
\caption{Target-Preservation Branch Evidence and Claim Boundaries}
\label{tab:gate-preservation-summary}
\centering
\scriptsize
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.17\textwidth}p{0.19\textwidth}p{0.16\textwidth}p{0.14\textwidth}p{0.22\textwidth}@{}}
\toprule
Policy & Evidence used & Detection result & Target-loss result & Reviewer-facing interpretation \\
\midrule
Raw map & Raw target evidence only & Mean $P_{\mathrm{D}}=0.145$ & 0.000 dB & No target-loss penalty, but weak detector evidence after calibration. \\
Low-rank residual & Residual evidence only & Mean $P_{\mathrm{D}}=0.256$ & 6.191 dB & Better detection than raw on the public diagnostic, but with substantial target attenuation. \\
Oracle blend diagnostic & Target-informed raw-residual blend & Mean $P_{\mathrm{D}}=0.359$ & 2.726 dB & Shows headroom for preservation-aware fusion, but is not a deployable detector. \\
Oracle gate diagnostic & Target-informed gate behavior & $P_{\mathrm{D}}=1.000$ at loose operating points & Near-zero loss & Establishes that the failure mode is correctable when target preservation is known. \\
Trainable TP-SSCS gate & Local raw and residual features with fixed saved state & Mean $P_{\mathrm{D}}=0.253$ & 0.197 dB & Deployable branch that keeps residual-level detection behavior while removing most target-loss cost. \\
\bottomrule
\end{tabular}
\vspace{0.35em}
\parbox{0.94\textwidth}{\scriptsize Table~\ref{tab:gate-preservation-summary} separates deployable trainable behavior from oracle diagnostics and shows that the selected gate reduces the target-loss cost of residual-only scoring.}
\end{table*}

\subsection{Official AISTAP-SIM Full-Asset Validation}

The central in-domain result evaluates a fixed TP-SSCS detector policy on \texttt{simMed\_test.mat} and \texttt{simWind\_test.mat}. The same saved state and detector policy are used for both assets. No asset-specific retuning is applied. The combined protocol includes 210 target-bearing frames and seven false-alarm operating points. Table~\ref{tab:aistap-full-asset-results} reports the combined numerical results, Table~\ref{tab:aistap-bootstrap-ci} reports paired bootstrap support, and Fig.~\ref{fig:official-full-asset-results} visualizes the operating curves, paired bootstrap support, asset-level deltas, and false-alarm calibration.
...
\begin{table*}[!t]
\caption{Paired Bootstrap Support for TP-SSCS on the Combined AISTAP-SIM Full-Asset Protocol}
\label{tab:aistap-bootstrap-ci}
\centering
\footnotesize
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{@{}cp{0.23\textwidth}cp{0.23\textwidth}c@{}}
\toprule
Target $P_{\mathrm{FA}}$ & $\Delta P_{\mathrm{D}}$ vs. Raw (95\% CI) & Positive fraction & $\Delta P_{\mathrm{D}}$ vs. Low-rank (95\% CI) & Positive fraction \\
\midrule
$1{\times}10^{-5}$ & 0.1046 [0.0942, 0.1152] & 0.895 & 0.0831 [0.0751, 0.0909] & 0.929 \\
$3{\times}10^{-5}$ & 0.1148 [0.1019, 0.1278] & 0.890 & 0.0788 [0.0711, 0.0871] & 0.910 \\
$1{\times}10^{-4}$ & 0.1633 [0.1471, 0.1783] & 0.895 & 0.0656 [0.0586, 0.0732] & 0.867 \\
$3{\times}10^{-4}$ & 0.2452 [0.2260, 0.2659] & 0.952 & 0.0539 [0.0471, 0.0619] & 0.733 \\
$1{\times}10^{-3}$ & 0.3258 [0.3034, 0.3486] & 0.976 & 0.0436 [0.0371, 0.0505] & 0.605 \\
$3{\times}10^{-3}$ & 0.3266 [0.3035, 0.3494] & 0.943 & 0.0353 [0.0292, 0.0417] & 0.519 \\
$1{\times}10^{-2}$ & 0.2127 [0.1917, 0.2349] & 0.895 & 0.0290 [0.0228, 0.0353] & 0.457 \\
\bottomrule
\end{tabular}
\vspace{0.35em}
\parbox{0.94\textwidth}{\scriptsize Table~\ref{tab:aistap-bootstrap-ci} reports paired uncertainty over the 210 target-bearing AISTAP-SIM frames and is used to check that the full-asset gains are not isolated-point effects.}
\end{table*}

\clearpage
\WideFigureBlock{../figures/submitted/figure7_official_full_asset_validation_20260716.pdf}{0.82\textwidth}{fig:official-full-asset-results}{Official AISTAP-SIM full-asset detector validation. (a) Combined operating curves over \texttt{simMed\_test.mat} and \texttt{simWind\_test.mat} show TP-SSCS, raw maps, and rank-matched low-rank residuals across seven target false-alarm probabilities. (b) Paired bootstrap confidence intervals over 210 target-bearing frames show positive mean $\Delta P_{\mathrm{D}}$ for TP-SSCS against both comparators. (c) Asset-level heatmap shows positive $\Delta P_{\mathrm{D}}$ on both official full-test assets. (d) Empirical false-alarm rates remain within the protocol ceiling across the combined full-asset operating points.}{Fig.~\ref{fig:official-full-asset-results} is the strongest visual evidence in the manuscript. It shows that the target-preserving detector policy is not merely a public-sample diagnostic. It improves low-false-alarm detection over raw and low-rank baselines on two official full-test conditions while preserving conservative false-alarm calibration. Tables~\ref{tab:gate-preservation-summary}--\ref{tab:aistap-bootstrap-ci} provide the corresponding numerical chain: branch-level preservation, full-asset operating values, and paired bootstrap support.}
...
The in-domain evidence in Tables~\ref{tab:gate-preservation-summary}--\ref{tab:aistap-bootstrap-ci} closes the primary AISTAP-SIM claim before the paper turns to external behavior. Table~\ref{tab:gate-preservation-summary} explains why the trainable branch is not merely an oracle diagnostic, Table~\ref{tab:aistap-full-asset-results} reports the fixed-policy operating values, and Table~\ref{tab:aistap-bootstrap-ci} gives the paired uncertainty support.
...
The unit-level bootstrap analysis is the main reason the SSDD result is credible as external support. Aggregate pixel-level gains can be driven by a small number of large or easy images. The image-level intervals show that the improvement is distributed across the 231 official-test images at the non-fallback points. The annotation-level intervals show that the same qualitative pattern holds across the 545 ship instances. This does not make SSDD a zero-shot transfer result, but it does show that supervised trainable-gate adaptation is not merely a pooled-pixel artifact. Table~\ref{tab:ssdd-unit-bootstrap} reports the image- and annotation-level intervals, while Table~\ref{tab:external-validation-summary}, Fig.~\ref{fig:external-radar-results}, and Fig.~\ref{fig:ssdd-unit-robustness} summarize the external evidence without expanding the claim beyond validation-selected or supervised adaptation.

\begin{table*}[!t]
\caption{SSDD Image- and Annotation-Level Bootstrap Support at Non-Fallback Operating Points}
\label{tab:ssdd-unit-bootstrap}
\centering
\scriptsize
\renewcommand{\arraystretch}{1.12}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}cp{0.23\textwidth}p{0.25\textwidth}p{0.13\textwidth}p{0.14\textwidth}@{}}
\toprule
Target $P_{\mathrm{FA}}$ & Image-level $\Delta P_{\mathrm{D}}$ vs. Raw (95\% CI) & Annotation-level $\Delta P_{\mathrm{D}}$ vs. Raw (95\% CI) & Image nonnegative fraction & Annotation nonnegative fraction \\
\midrule
$3{\times}10^{-4}$ & 0.1222 [0.0986, 0.1449] & 0.0797 [0.0664, 0.0937] & 0.870 & 0.906 \\
$1{\times}10^{-3}$ & 0.1596 [0.1345, 0.1852] & 0.0820 [0.0663, 0.0979] & 0.861 & 0.818 \\
$3{\times}10^{-3}$ & 0.2448 [0.2178, 0.2720] & 0.1680 [0.1485, 0.1882] & 0.918 & 0.872 \\
$1{\times}10^{-2}$ & 0.2182 [0.1962, 0.2386] & 0.1885 [0.1720, 0.2045] & 0.913 & 0.892 \\
\bottomrule
\end{tabular}
\vspace{0.35em}
\parbox{0.94\textwidth}{\scriptsize Table~\ref{tab:ssdd-unit-bootstrap} reports SSDD support at image and annotation units, preventing the SAR adaptation result from being interpreted as only a pooled-pixel gain.}
\end{table*}

\begin{table*}[!t]
\caption{External Validation and Adaptation Summary}
\label{tab:external-validation-summary}
\centering
\footnotesize
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}p{0.20\textwidth}p{0.20\textwidth}p{0.17\textwidth}p{0.35\textwidth}@{}}
\toprule
Protocol & Adaptation policy & Test unit & Main outcome and claim boundary \\
\midrule
IPIX direct zero-shot transfer & Saved AISTAP-SIM TP-SSCS state applied without IPIX validation selection & IPIX transfer check & Negative boundary result. At $P_{\mathrm{FA}}=10^{-2}$, raw evidence reaches $P_{\mathrm{D}}=0.0364$ while the transferred detector reaches 0.0086; unmodified zero-shot superiority is not supported. \\
IPIX validation-selected fusion & Residual-aware fusion with $\beta=1.5$ selected on one validation recording and frozen for testing & 12 held-out recordings & Positive bounded adaptation. Fusion wins all seven operating points against raw and low-rank residuals; at $P_{\mathrm{FA}}=10^{-2}$ it reaches $P_{\mathrm{D}}=0.1374$, compared with 0.0972 for raw and 0.0096 for low-rank residuals. \\
SSDD supervised trainable gate & Gate trained on the official train split, selected by validation, and evaluated on the official test split & 231 images / 545 ship annotations & Supervised external adaptation. The candidate records four wins and three raw-fallback ties against raw, and seven wins against low-rank residuals; at $P_{\mathrm{FA}}=10^{-2}$ it reaches $P_{\mathrm{D}}=0.7469$, compared with 0.5284 and 0.0195. \\
SSDD unit-level robustness & Fixed global thresholds with image- and annotation-level resampling & Images and annotations & Image-level $\Delta P_{\mathrm{D}}$ versus raw is 0.2182 with 95\% CI [0.1962, 0.2386] at $P_{\mathrm{FA}}=10^{-2}$; annotation-level resampling shows the same positive non-fallback pattern. \\
\bottomrule
\end{tabular}
\vspace{0.35em}
\parbox{0.94\textwidth}{\scriptsize Table~\ref{tab:external-validation-summary} defines the external claim boundary by separating negative zero-shot transfer, validation-selected IPIX fusion, and supervised SSDD adaptation.}
\end{table*}

\clearpage
\WideFigureBlock{../figures/submitted/figure8_external_radar_validation_20260716.pdf}{0.84\textwidth}{fig:external-radar-results}{Bounded external radar-family validation. (a) IPIX held-out recordings compare validation-selected residual-aware fusion against raw and low-rank residual baselines. (b) Recording-level bootstrap intervals summarize IPIX $\Delta P_{\mathrm{D}}$ against both comparators. (c) SSDD official-test SAR ship imagery compares the supervised adaptation candidate with raw and low-rank baselines. (d) SSDD image-level bootstrap intervals show raw-fallback no-regression at the most stringent points and positive learned-gate gains at non-fallback operating points.}{Fig.~\ref{fig:external-radar-results} is included to keep the external evidence bounded. The IPIX panels show validation-selected fusion rather than zero-shot detector transfer, and the SSDD panels show supervised adaptation with conservative raw fallback in the extreme false-alarm tail.}

\clearpage
\WideFigureBlock{../figures/submitted/supplementary_figure1_ssdd_robustness_20260716.pdf}{0.84\textwidth}{fig:ssdd-unit-robustness}{SSDD image- and annotation-level robustness analysis. Image-level and annotation-level boxplots show the distribution of SSDD detection-probability gains against raw at non-fallback false-alarm probabilities and against low-rank residuals across all operating points. Box centers show medians, boxes show interquartile ranges, whiskers exclude plotted outliers, and filled circles mark means.}{Fig.~\ref{fig:ssdd-unit-robustness} checks whether the SSDD gain is distributed over meaningful units rather than produced only by pooled pixels. This is why Tables~\ref{tab:ssdd-unit-bootstrap} and \ref{tab:external-validation-summary} report image-level, annotation-level, and protocol-level evidence separately.}

\clearpage
\WideFigureBlock{../figures/submitted/figure6_evidence_hierarchy.png}{0.84\textwidth}{fig:evidence-hierarchy}{Evidence hierarchy and claim boundaries across radar domains. AISTAP-SIM provides the strongest controlled in-domain detector evidence, while IPIX and SSDD provide bounded external support under validation-selected fusion and supervised adaptation, respectively.}{Taken together, the three result layers support a coherent evidence hierarchy. The public-sample audit identifies the failure mode. The trainable gate shows that the target-preservation correction is concrete and reproducible. The official AISTAP-SIM full-asset protocol provides the main detector validation. IPIX and SSDD then test external behavior under bounded adaptation protocols. This hierarchy is stronger than presenting one aggregate leaderboard because each layer answers a different reviewer question. Fig.~\ref{fig:evidence-hierarchy} makes the claim boundaries explicit across AISTAP-SIM, IPIX, and SSDD.}
...
The method also clarifies the role of figures and tables in the final manuscript. Residual maps can illustrate qualitative behavior, but they should not be used as the primary proof. The central evidence should remain operating tables, target-loss summaries, paired bootstrap intervals, and external protocol summaries. Tables~\ref{tab:ssdd-unit-bootstrap} and \ref{tab:external-validation-summary} are included in the main text for this reason: they turn the SSDD and IPIX external results into unit-aware, claim-bounded evidence rather than a pooled leaderboard. Visual examples can help readers understand failure modes, but the detector claim must rest on calibrated numeric comparisons.
...
Another limitation is that the current manuscript uses placeholder references. The experimental package is internally traceable, but TGRS submission requires verified citations for STAP, CFAR, low-rank suppression, AISTAP-SIM, IPIX, SSDD, SAR ship detection, and weak-target learning. Replacing placeholders with DOI-verified references is a manuscript-readiness task, not an experimental task, but it is essential before submission.
...
\bibitem{liu2020objectsurvey}
L. Liu, W. Ouyang, X. Wang, P. Fieguth, J. Chen, X. Liu, and M. Pietikainen, ``Deep learning for generic object detection: A survey,'' \emph{International Journal of Computer Vision}, vol.~128, no.~2, pp.~261--318, Feb. 2020, doi: 10.1007/s11263-019-01247-4.

\bibitem{ren2017fasterrcnn}
S. Ren, K. He, R. Girshick, and J. Sun, ``Faster R-CNN: Towards real-time object detection with region proposal networks,'' \emph{IEEE Transactions on Pattern Analysis and Machine Intelligence}, vol.~39, no.~6, pp.~1137--1149, Jun. 2017, doi: 10.1109/TPAMI.2016.2577031.

\bibitem{redmon2016yolo}
J. Redmon, S. Divvala, R. Girshick, and A. Farhadi, ``You only look once: Unified, real-time object detection,'' in \emph{Proc. IEEE Conf. Computer Vision and Pattern Recognition (CVPR)}, 2016, pp.~779--788, doi: 10.1109/CVPR.2016.91.

\bibitem{shelhamer2017fcn}
E. Shelhamer, J. Long, and T. Darrell, ``Fully convolutional networks for semantic segmentation,'' \emph{IEEE Transactions on Pattern Analysis and Machine Intelligence}, vol.~39, no.~4, pp.~640--651, Apr. 2017, doi: 10.1109/TPAMI.2016.2572683.

\bibitem{ronneberger2015unet}
O. Ronneberger, P. Fischer, and T. Brox, ``U-Net: Convolutional networks for biomedical image segmentation,'' in \emph{Medical Image Computing and Computer-Assisted Intervention -- MICCAI 2015}, 2015, pp.~234--241, doi: 10.1007/978-3-319-24574-4\_28.
