...
These comparators were selected to answer a specific question: whether target-preserving structured-clutter suppression improves detection over both raw evidence and a strong residual baseline under the same false-alarm constraint. Additional classical detectors can be added in future revisions, but only if they are implemented under identical calibration rules.
...
\begin{figure*}[!t]
\centering
\FigureOrPlaceholder{../figures/submitted/figure6_evidence_hierarchy.png}{0.92\textwidth}
\caption{Evidence hierarchy and claim boundaries across radar domains. AISTAP-SIM provides the strongest controlled in-domain detector evidence, while IPIX and SSDD provide bounded external support under validation-selected fusion and supervised adaptation, respectively.}
...
Finally, the current draft does not yet include figures or final tables. That is intentional for the present writing stage, but a TGRS submission will need visual and tabular support. The figures should show the suppression-preservation trade-off, the official AISTAP-SIM operating curves, and bounded external validation behavior. Tables should report operating-point metrics, bootstrap intervals, and dataset/protocol roles. The text has been expanded so that these elements can be inserted without changing the central argument.

\section{Conclusion}

This paper reformulates structured-clutter suppression as a target-preserving low-false-alarm radar detection problem. Low-rank audits on AISTAP-SIM show that stronger clutter attenuation can substantially increase target loss, making residual cleanliness an incomplete evaluation criterion. TP-SSCS addresses this issue with a trainable target-preservation gate and conservative false-alarm-calibrated operating analysis.

The main evidence is a fixed detector-policy evaluation on two official AISTAP-SIM full-test assets. Across 210 target-bearing frames and seven false-alarm operating points, TP-SSCS improves over raw maps and rank-matched low-rank residuals at every operating point, with positive paired bootstrap confidence intervals. External radar-family tests further bound the claim: direct zero-shot IPIX transfer is negative, validation-selected residual-aware fusion improves held-out IPIX recordings, and supervised trainable-gate adaptation improves official-test SSDD SAR ship detection. These results support TP-SSCS as a target-preserving detector design for low-false-alarm radar operation, while leaving production deployment and unqualified cross-dataset transfer for future work.

The broader implication is that clutter suppression should be evaluated as part of detector operation. A cleaner residual is useful only if it improves target detection at the required false-alarm rate. TP-SSCS provides one reproducible way to enforce this principle by combining residual structure, target-preserving gating, and calibrated operating analysis. The resulting evidence is strongest for AISTAP-SIM official full-test validation and supportive, with stated boundaries, for IPIX and SSDD external adaptation.
