...
def make_figure(df: pd.DataFrame, out_path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(7.4, 5.4), constrained_layout=False)
    colors = {
        "raw": "#666666",
    ...
    x_max = float(summary["target_loss_mean"].max())
    x_mid = float(summary["target_loss_mean"].median())
    handles = []
    for _, row in summary.iterrows():
        label = row["method"]
        x_val = float(row["target_loss_mean"])
        y_val = float(row["pd_mean"])
        handle = ax.scatter(
            x_val,
            y_val,
            s=110,
            ...
            label={
                ...
                "trainable_gate": "trainable gate",
                "oracle_gate": "oracle gate",
    ...
    ax.set_title("Target-preservation frontier on the public AISTAP-SIM sample", pad=10)
    ax.grid(True, alpha=0.25)
    ax.set_xlim(left=min(-0.35, float(summary["target_loss_mean"].min()) - 0.35), right=float(summary["target_loss_mean"].max()) + 0.9)
