...
def gate_cross_condition(root: Path) -> Gate:
    full_json = latest(sorted((root / "logs").glob("aistap_cross_condition_full_asset_validation_*.json")))
    full_md = latest(sorted((root / "logs").glob("aistap_cross_condition_full_asset_validation_*.md")))
    full_csv = latest(sorted((root / "results" / "aistap_full_asset").glob("aistap_cross_condition_full_asset_summary_*.csv")))
    if full_json is not None and full_csv is not None:
        try:
            ...
            return Gate(
                ...
                status="partial",
                evidence=str(full_json.relative_to(root)),
                detail=f"could not parse official full-asset cross-condition JSON: {exc}",
            )
        criteria = data.get("criteria", {})
        assets = criteria.get("assets", [])
        comparison_count = int(criteria.get("comparison_count", len(comparisons)))
        all_win_raw = all(bool(row.get("beats_raw")) for row in comparisons)
        all_win_lowrank = all(bool(row.get("beats_lowrank")) for row in comparisons)
        all_pfa_calibrated = all(bool(row.get("pfa_calibrated")) for row in comparisons)
        min_items = min((int(row.get("n_items", 0)) for row in comparisons), default=0)
        ok = (
            data.get("passed") is True
            and len(assets) >= 2
            and comparison_count >= 14
            and min_items >= 100
            and all_win_raw
            and all_win_lowrank
            and all_pfa_calibrated
        )
        evidence = [str(full_json.relative_to(root)), str(full_csv.relative_to(root))]
        if full_md is not None:
            evidence.append(str(full_md.relative_to(root)))
        return Gate(
            name="cross_condition_holdout",
            status="pass" if ok else "partial",
            evidence=", ".join(evidence),
            detail=(
                "official full-asset cross-condition validation across "
                f"{', '.join(str(x) for x in assets)}: comparison_count={comparison_count}, "
                f"minimum target-bearing items per asset={min_items}, "
                f"all beats raw={all_win_raw}, all beats low-rank={all_win_lowrank}, "
                f"all empirical-Pfa calibrated={all_pfa_calibrated}; "
                "bounded to AISTAP-SIM official conditions, not independent non-AISTAP transfer"
            ),
        )
